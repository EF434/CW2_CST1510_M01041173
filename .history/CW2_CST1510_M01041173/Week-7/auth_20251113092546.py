...
# Decorative header for the platform
def display_header():
    print("\n" + "="*70)
    print("🧠 MULTI-DOMAIN INTELLIGENCE PLATFORM 🧠".center(70))
    print("🔒 Secure Authentication System 🔒".center(70))
    print("="*70 + "\n")
...
def display_menu():
    ...
    for option in menu:
        print(option.center(70))
        time.sleep(0.2)
    print("-"*70)

# Example placeholder functions for username/password validation
def validate_username(username):
...
def login_user(username, password):
    if username in users_db and users_db[username] == password:
        print(f"✅ Welcome back, {username}! You are logged in successfully!")
        return True
    else:
...
def main():
    print("\n🎉 Welcome to the Week 7 Authentication System! 🎉".center(70))
    while True:
        display_menu()
        choice = input("Select an option (1-3): ".center(70) + "\n> ").strip()
        print("\n" + "-"*70)

        if choice == '1':
            # Registration flow
            print("📝 --- USER REGISTRATION --- 📝".center(70))
            username = input("Enter a username: ".center(70)).strip()
            is_valid, msg = validate_username(username)
            if not is_valid:
                print(f"⚠️ {msg}")
                continue
            password = input("Enter a password: ".center(70)).strip()
            is_valid, msg = validate_password(password)
            if not is_valid:
                print(f"⚠️ {msg}")
                continue
            password_confirm = input("Confirm password: ".center(70)).strip()
            if password != password_confirm:
                print("⚠️ Passwords do not match.")
                continue
            register_user(username, password)
            input("\nPress Enter to return to the main menu...".center(70))
        
        elif choice == '2':
            # Login flow
            print("🔐 --- USER LOGIN --- 🔐".center(70))
            username = input("Enter your username: ".center(70)).strip()
            password = input("Enter your password: ".center(70)).strip()
            if login_user(username, password):
                input("\nPress Enter to return to the main menu...".center(70))
        
        elif choice == '3':
            # Exit
            print("\nThank you for using the authentication system! 👋".center(70))
            print("Exiting...".center(70))
            time.sleep(1)
            break
        
        else:
            print("\n⚠️ Invalid option. Please select 1, 2, or 3.".center(70))
            time.sleep(1)
