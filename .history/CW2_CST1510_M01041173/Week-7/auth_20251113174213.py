...
def retrieve_attempts_count():
    ...
    with open(USER_LOCK_FILE, "r") as f:
        count = f.read().strip()
        return int(count) else raise ValueError("Invalid data in attempts file")



def save_failed_attempts(count):
    """Save the current attempt count to the file."""
...
def record_failed_attempt():
    ...
    if count >= MAX_ATTEMPTS:
        ...
        for remaining in range(LOCK_DURATION, 0, -1):
