...
# Initialize lockout constants
USER_LOCK_FILE = "user_lock_data.txt"
MAX_ATTEMPTS = 3
...
def register_user(username: str, password: str, role: str = "user") -> tuple[bool, str, str]:
    '''
...
def add_failed_attempt():
    ...
    with open(USER_LOCK_FILE, "r") as f:
        attempts_count = f.read().strip()
        return int(attempts_count)

    attempts_count = retrieve_attempts_count() + 1
    user_attempts(attempts_count)

def retrieve_attempts_count():
    """Retrieve the current number of failed attempts from the file."""

    # If the file doesn't exist, return 0
    if not os.path.exists(USER_LOCK_FILE):
...
# Save the current number of failed attempts to the file
def user_attempts(attempts_count):
    """Save the current number of failed attempts to the file."""
    with open(USER_LOCK_FILE, "w") as f:
        f.write(str(attempts_count))


def add_failed_attempt():
    """Increment the failed attempts count."""
    count = retrieve_attempts_count() + 1
    add_failed_attempt(count)

    if attempts_count >= MAX_ATTEMPTS:
        print("🚫 Too many failed attempts. Account locked for 5 minutes.\n")
        for remaining in range(ACCOUNT_LOCK_TIME, 0, -1):
            mins, secs = divmod(remaining, 60)
            print(f"\r🔒 Try again in {mins}m {secs}s", end="")
            time.sleep(1)
        print("\n✅ Account unlocked. You can try again now.")
        add_failed_attempt(0)  # reset after lock
    else:
        remaining = MAX_ATTEMPTS - count
        print(f"⚠️ Attempt {count}/{MAX_ATTEMPTS}. {remaining} tries left.")


def reset_attempts():
    """Reset failed attempts after a successful login."""
    add_failed_attempt(0)
