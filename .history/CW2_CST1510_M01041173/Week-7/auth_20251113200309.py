...
# Initialize or update failed attempts dictionary
failed_attempts = {}
...
def add_failed_attempt(username):
    ...
    if username in failed_attempts:
        attempts_count, last_time = failed_attempts[username]
    else:
        attempts_count = 0
        last_time = 0
    ...
    # Increment attempts
    attempts_count += 1
    failed_attempts[username] = (attempts_count, current_time)


    # Unpack attempts count and last attempt time
    print(f"Failed attempt {failed_attempts[username][0]} of {MAX_ATTEMPTS} for '{username}'.")
    ...
    print(f"⚠️ Failed attempt {attempts} of {MAX_ATTEMPTS} for '{username}'.")

    # Lock account if max reached
    if attempts >= MAX_ATTEMPTS:
        print(f"🚨 Too many failed attempts! Account locked for {LOCKOUT_TIME // 60} minutes.")
