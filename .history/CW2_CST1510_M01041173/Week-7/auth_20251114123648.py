...
def register_user(username: str, password: str, role: str = "user") -> bool:
    ...
    role = input("Enter role for the new user (default 'user'): ").strip().lower()

    if role not in available_roles or role == "":
        print(f"Error: Invalid role '{role}'. Available roles: {', '.join(available_roles)}.")
        return False
