...
# Import additional modules for challenges
import string # For Challenge 1
...
def register_user(username: str, password: str, role: str = "user") -> bool:
    ...
    # Ask user for role input (Challenge 2)
    available_roles = ["user", "admin", "analyst"]
    attempts_count = 3
    while attempts_count > 0:
        print(f"Available roles: {', '.join(available_roles)}")
        role = input("Enter role for {username}: ").strip().lower()
 
        if role not in available_roles:
            break
        else:
            attempts_count -= 1
            print(f"Error: Invalid role '{role}'. Attempts remainig {attempts_count}")
    if attempts_count == 0:
        print("Error: Maximum attempts reached for role selection. Defaulting to 'user'.")
        role = "user"
...
def login_user(username, password):
    ...
    # Prevent login attempt during lockout
            ...
            for line in f.readlines():
                ...
                if user == username:
                    ...
                    if verify_password(password, hash):
                        ...
                        failed_attempts[username] += 1  # Increment failed attempts
                        remaining = MAX_ATTEMPTS - failed_attempts[username]
                        if remaining > 0:
                            return f"❌ Incorrect password. Attempts used: {failed_attempts[username]}. Attempts remaining: {remaining}"
                        else:
                            return f"❌ Incorrect password. Attempts used: {failed_attempts[username]}. Account locked for {ACCOUNT_LOCK_TIME // 60} minutes."
    ...
    while True:
        ...
        # -------------------- Login --------------------
            ...
            while login_attempts < MAX_ATTEMPTS:
                ...
                print(f"❌ Maximum login attempts reached. Please wait {ACCOUNT_LOCK_TIME // 60} minutes before trying again.")
