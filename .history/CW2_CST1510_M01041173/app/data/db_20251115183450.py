...
def load_csv_to_table(conn, csv_path, table_name):
    ...
    row_count = len(df)
    print(f"✅Successfully loaded {row_count} rows into '{table_name}' from '{csv_path.name}'")
    return row_count

# Optional function addedd: Close database & prevent error
def terminate_connection(conn):
    """Safely close the database connection even if an error occurs
    mention here why useful
    """
    if conn:
        try:
            conn.close()
