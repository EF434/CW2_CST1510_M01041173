...
def login_user(username, password):
    ...
    if username in account_lock:
        ...
        remaining_attempts = int(account_lock[username] - current_time)
        if remaining_attempts == 0:
            minutes = remaining_attempts // 60
            seconds = remaining_attempts % 60
            return False, f"🔒 '{user_name}' Account locked. Try again in {minutes}m {seconds}s"
        else:
            # Unlock account
            failed_attempts[username] = 0
            del account_lock[username]
    ...
    if remaining_attempts > 0:
        ...
        lockout_end_time[username] = time.time() + ACCOUNT_LOCK_TIME
        return False, "🚫 Maxiam failed attempts reached. Account locked for 5 minutes."
