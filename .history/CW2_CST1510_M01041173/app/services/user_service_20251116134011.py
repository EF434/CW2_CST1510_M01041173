...
def migrate_users_from_file(conn, filepath= DATA_DIR / "users.txt"):
    ...
    """
    # Determine project root (where main.py lives)
    PROJECT_ROOT = Path(__file__).resolve().parents[1]  # Adjust if your file is nested
    ...
    with open(filepath, 'r') as f:
        for line in f:
            ...
            if len(parts) >= 2:
                ...
                if len(user_data) >= 3 and user_data[2].lower() in AVAILABLE_ROLES:
                    role = user_data[2].lower()
                else:
                    role = 'user'
