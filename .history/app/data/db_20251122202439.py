...
# -----------------------------------
# Load CSV data into a database table (only if empty)
# -----------------------------------
...
def load_all_csv_data(conn, csv_path, table_name):
    ...
    Only loads data if the table is empty to avoid overwriting user-added records.
    Resets AUTOINCREMENT if the table was empty.
    """
    try:
        ...
        # Drop 'id' column if exists (so SQLite autogenerates it)
        if "id" in df.columns:
