...
def insert_ticket(conn, ticket_id, priority, status, category, subject, description,
    ...
    Args:
        ...
        ticket_id: Unique ticket reference (TEXT)
        priority: Priority level (e.g., Critical, High, Medium, Low)
        status: Current status (e.g., Open, Pending, Resolved, Closed)
        category: Type of issue (Hardware, Software, Network, Account, etc.)
        subject: Short title or summary
        description: Full ticket message
        created_date: Date ticket was created (optional)
...
def delete_ticket(conn, ticket_id):
    ...
    try:
        ...
        print(f"🗑️ Ticket '{ticket_id}' deleted successfully.")
        return cursor.rowcount
