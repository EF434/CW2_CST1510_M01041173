how is this code supposed to work import streamlit as st
from auth import (
    register_user,
    login_user,
    user_exists,
...
if "account_created" not in st.session_state:
    st.session_state.account_created = False  # Flag to switch to Login tab
    ...
    if st.session_state.active_tab == "Login":
        ...
        if st.button("Log in", type="primary"):
            if not USER_DATA_FILE.exists() or not user_exists(login_username):
                st.warning(f"⚠️ Username '{login_username}' not found. Please register first.")
