...
# --------- Step 7. Implement the Registration Function def register_user(username: str, password: str) -> tuple[bool, str]: ''' Registers a new user by hashing their password and storing credentials. Args: username (str): The username for the new account password (str): The plaintext password to hash and store Returns: bool: True if registration successful, False if username already exists ''' # Check for empty or whitespace-only input if not username.strip() or not password.strip(): return False, "Username and password cannot be empty." # Check if the username already exists if user_exists(username): return False, f"Error: Username '{username}' already exists." # Hash the password hashed_password = hash_password(password) # Append the new user to the file # Format: username,hashed_password try: with open("users.txt", "a") as f: f.write(f"{username},{hashed_password}\n") print(f"User '{username}' registered.") return True except ValueError: print("Error: Invalid input for registration.") return False except Exception as e: print(f"Error during user registration: {e}") return False   
