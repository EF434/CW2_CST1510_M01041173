...
def register_user(username: str, password: str) -> tuple[bool, str]: 
    ...
    # if user_exists(username): return False, f"Error: Username '{username}' already exists." # Hash the password hashed_password = hash_password(password) # Append the new user to the file # Format: username,hashed_password try: with open("users.txt", "a") as f: f.write(f"{username},{hashed_password}\n") print(f"User '{username}' registered.") return True except ValueError: print("Error: Invalid input for registration.") return False except Exception as e: print(f"Error during user registration: {e}") return False   
