...
filter_columns = {
    ...
    "IT Operations": {"priority": ["Low", "Medium", "High", "Critical"],
                      ...
                      "category": ["Biology", "Computer Networks", "Transportation", "Climate/Weather"]},
    "Data Science": {}
}
...
# 1️⃣ Predefined dropdown filters
domain_filters = filter_columns.get(domain_choice, {})
for col, options in domain_filters.items():
    if col in df.columns:
        selected = st.sidebar.multiselect(f"Filter by {col}", options)
        if selected:
            df_filtered = df_filtered[df_filtered[col].isin(selected)]

# 2️⃣ Custom filter: user types column name & value
st.sidebar.markdown("### Custom Filter")
custom_col = st.sidebar.text_input("Enter column name to filter")
custom_val = st.sidebar.text_input("Enter value to filter (exact match)")

if custom_col and custom_val:
    if custom_col in df.columns:
        df_filtered = df_filtered[df_filtered[custom_col] == custom_val]
    else:
        st.sidebar.warning(f"Column '{custom_col}' not found in this domain.")
...
for action in actions:
    if action == "● View Records":
        view_records(domain_choice, df_filtered, domain_filters)
