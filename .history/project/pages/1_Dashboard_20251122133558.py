...
# ------------------- PAGE FILTERING -------------------
st.markdown("### Filters")
st.info("Select from the dropdowns or enter an exact column name and value. Column names must match exactly!")

# Start with a copy of the dataframe
df_filtered = df.copy()

# 1️⃣ Predefined dropdown filters (on page)
...
# 2️⃣ Custom filter: user types column name & value (on page)
...
if custom_col or custom_val:
    ...
    elif custom_val:  # only filter if column exists and value is entered
