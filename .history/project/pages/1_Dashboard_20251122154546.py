...
def refresh_data():
    """Reload domain data into session state."""
    config = ADD_CONFIG[domain_choice]
    if "get_func" in config and config["get_func"]:
        st.session_state.df = pd.DataFrame(config["get_func"](conn))
    else:
...
for action in actions:
    if action == "● View Records":
        view_records(domain_choice)

    elif action == "● Add New Record":
        config = ADD_CONFIG.get(domain_choice)
        if config:
            add_new_record(domain_name=domain_choice,
                           insert_func=config["insert_func"],
        ...
        config = ADD_CONFIG.get(domain_choice)
        if config:
            update_delete_record(domain_name=domain_choice,
                                 df=df,
                                 update_func=config["update_func"],
                                 delete_func=config["delete_func"],
                                 fields=config["form_fields"])
            refresh_data()
        ...
        if df.empty:
            ...
            # Placeholder for charts, analytics functions can be called here
            st.write("Analytics/Charts will be displayed here.")
# Logout button
st.divider()
