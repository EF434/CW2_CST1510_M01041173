    ...
    if domain == "Cybersecurity":
        ...
        with tab1:
            ...
            fig1 = px.bar(df_type_count, x="incident_type", y="count", color="incident_type",
                          text="count", title="Total Incidents by Type")
...
with col1:
    st.metric(
        label="Total Tickets",
        value=len(df),
        delta=f"{len(df[df['is_resolved']])} Resolved"
    )

with col2:
    avg_res_time = round(df["resolution_time_days"].mean(), 2)
    st.metric(
        label="Average Resolution Time (days)",
        value=avg_res_time,
        delta=f"{round(df[df['is_resolved']]['resolution_time_days'].mean(),2)} Resolved"
    )

with col3:
    unresolved_count = len(df[~df["is_resolved"]])
    st.metric(
        label="Unresolved Tickets",
        value=unresolved_count,
        delta=f"{round(unresolved_count / len(df) * 100, 1)}%"
    )

# ------------------- DELAYS BY STAFF -------------------
st.subheader("📊 Staff Performance (Delays)")

staff_df = df.groupby("assigned_to").agg(
    total_tickets=("ticket_id", "count"),
    unresolved_tickets=("is_resolved", lambda x: (~x).sum()),
    avg_resolution_days=("resolution_time_days", "mean")
).sort_values("avg_resolution_days", ascending=False).reset_index()
...
fig_staff = px.bar(
    ...
    title="Average Ticket Resolution Time by Staff (days)",
...
# ------------------- DELAYS BY STATUS -------------------
st.subheader("⏳ Process Bottlenecks by Status")
...
fig_status = px.bar(
    ...
    y="avg_resolution_days",
    text="avg_resolution_days",
    title="Average Ticket Resolution Time by Status (days)",
    color="avg_resolution_days",
    color_continuous_scale="Blues"
)
st.plotly_chart(fig_status, use_container_width=True)

# ------------------- HEATMAP: Staff vs Status -------------------
st.subheader("🔥 Heatmap: Staff vs Status")
...
fig_heat = px.imshow(
    ...
    text_auto=True,
    aspect="auto",
    title="Ticket Count by Staff and Status",
    color_continuous_scale="Viridis"
)
st.plotly_chart(fig_heat, use_container_width=True)

# ------------------- LIVE UPDATE SIMULATION -------------------
st.subheader("⚡ Real-Time Updates Simulation")
placeholder = st.empty()

for i in range(1, 6):
    # Simulate metrics changing over time
    placeholder.metric("Tickets Resolved in Last Hour", value=i*2, delta="+2")
    st.sleep(1)
