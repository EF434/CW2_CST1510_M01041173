...
def it_tickets_func():
    ...
    with st.form("add_ticket_form"):
        ...
        submitted = st.form_submit_button("Add Ticket")
        if submitted:
            insert_ticket(conn, ticket_id, priority, status, category, subject,
                description, str(created_date), str(resolved_date),assigned_to
            )
            st.success(f"✓ dataset_ '{ticket_id}' added successfully!")
            st.rerun()
