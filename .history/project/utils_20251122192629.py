...
def datasets_metadata_func():
    ...
    def delete_dataset_ui():
        """
        Delete a dataset via Streamlit UI.
        """
        # Fetch all datasets
        datasets = get_all_datasets(conn)  # Should return a DataFrame
        if datasets.empty:
            st.info("No datasets available to delete.")
            return

        # Map datasets to display string (dataset_name + category)
        dataset_options = [f"{row['dataset_name']}: {row['category']}" for idx, row in datasets.iterrows()]

        # Select dataset to delete
        selected = st.selectbox("Select dataset to delete", dataset_options)
        dataset_name = selected.split(":")[0]  # Extract dataset_name

        # Confirmation checkbox
        confirm = st.checkbox(f"Confirm delete dataset '{dataset_name}'")
