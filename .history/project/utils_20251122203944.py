...
def update_delete_record(conn, table_name):
    if table_name == "cyber_incidents":
        ...
        selected = st.selectbox("Select incident to view/update/delete", options)
        ...
        # Update form (example fields)
        with st.form("update_incident_form"):
            new_status = st.selectbox("Status", ["Open", "In Progress", "Closed"], index=["Open","In Progress","Closed"].index(record['status']))
            new_priority = st.selectbox("Priority", ["Low", "Medium", "High"], index=["Low","Medium","High"].index(record['priority']))
        ...
        selected = st.selectbox("Select ticket to view/update/delete", options)
        ...
        with st.form("update_ticket_form"):
            new_status = st.selectbox("Status", ["Open", "In Progress", "Resolved"], index=["Open","In Progress","Resolved"].index(record['status']))
            if st.form_submit_button("Update Ticket"):
                updated_count = update_ticket(conn, ticket_id, new_status)
                if updated_count > 0:
        ...
        selected = st.selectbox("Select dataset to view/update/delete", options)
        ...
        with st.form("update_dataset_form"):
            ...
            if st.form_submit_button("Update Dataset"):
                updated_count = update_dataset(conn, dataset_name, new_category)
                if updated_count > 0:
