...
def update_delete_record(conn, table_name):
    if table_name == "cyber_incidents":
        ...
        selected = st.selectbox(
            "Select incident to view/update/delete",
            [f"{row['id']}: {row.get('incident_type', '')}" for _, row in df.iterrows()]
        )
        incident_id = int(selected.split(":")[0])
        record = df[df['id'] == incident_id].iloc[0]

        st.write("### Selected Incident Details")
        st.write(record.to_dict())

        with st.form("update_incident_form"):
            # Only create field if exists
            new_status = st.selectbox(
                "Status",
                ["Open", "In Progress", "Closed"],
                index=["Open","In Progress","Closed"].index(record['status']) if 'status' in record else 0
            )
            new_priority = st.selectbox(
                "Priority",
                ["Low", "Medium", "High"],
                index=["Low","Medium","High"].index(record['priority']) if 'priority' in record else 0
            )
            if st.form_submit_button("Update Incident"):
                updated_count = update_incident_status(conn, incident_id, new_status, new_priority)
                if updated_count > 0:
                    st.success(f"Incident {incident_id} updated successfully!")
                    st.rerun()
                else:
                    st.error(f"Failed to update incident {incident_id}")

        if st.button("Delete Incident"):
            if delete_incident(conn, incident_id) > 0:
        ...
        selected = st.selectbox(
            "Select ticket to view/update/delete",
            [f"{row['ticket_id']}: {row.get('subject', '')}" for _, row in df.iterrows()]
        )
        ticket_id = selected.split(":")[0]
        record = df[df['ticket_id'] == ticket_id].iloc[0]
        ...
        with st.form("update_ticket_form"):
            new_status = st.selectbox(
                ...
                index=["Open","In Progress","Resolved"].index(record['status']) if 'status' in record else 0
            )
            if st.form_submit_button("Update Ticket"):
                if update_ticket(conn, ticket_id, new_status) > 0:
                    st.success(f"Ticket {ticket_id} updated successfully!")
                    st.rerun()
                else:
                    st.error(f"Failed to update ticket {ticket_id}")

        if st.button("Delete Ticket"):
            if delete_ticket(conn, ticket_id) > 0:
        ...
        selected = st.selectbox(
            "Select dataset to view/update/delete",
            [f"{row['dataset_name']}: {row.get('category', '')}" for _, row in df.iterrows()]
        )
        dataset_name = selected.split(":")[0]
        record = df[df['dataset_name'] == dataset_name].iloc[0]

        st.write("### Selected Dataset Details")
        st.write(record.to_dict())

        with st.form("update_dataset_form"):
            new_category = st.text_input("Category", record['category'] if 'category' in record else "")
            if st.form_submit_button("Update Dataset"):
                if update_dataset(conn, dataset_name, new_category) > 0:
                    st.success(f"Dataset '{dataset_name}' updated successfully!")
                    st.rerun()
                else:
                    st.error(f"Failed to update dataset '{dataset_name}'")

        if st.button("Delete Dataset"):
            if delete_dataset(conn, dataset_name) > 0:
