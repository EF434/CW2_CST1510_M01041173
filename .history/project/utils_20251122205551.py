    ...
    with st.form("update_delete_form"):
        ...
        with col1:
            if st.form_submit_button("Update"):
                # Call existing update functions if possible
                if table_name == "cyber_incidents":
                    update_incident_status(conn, selected_id, updated_values.get("status", record.get("status")))
                elif table_name == "it_tickets":
                    update_ticket(conn, selected_id, updated_values.get("status", record.get("status")))
                elif table_name == "datasets_metadata":
                    update_dataset(conn, selected_id, updated_values.get("category", record.get("category")))
        ...
        with col2:
            if st.form_submit_button("Delete", type="primary"):
                ...
                if table_name == "cyber_incidents":
                    ...
                    delete_ticket(conn, selected_id)
                elif table_name == "datasets_metadata":
                    delete_dataset(conn, selected_id)
