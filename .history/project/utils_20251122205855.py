    ...
    with st.form("update_delete_form"):
        ...
        with col1:
            if st.form_submit_button("Update"):
                # --- Call update functions based on table ---
                if table_name == "cyber_incidents":
                    update_incident_status(
                        conn,
                        selected_id,
                        updated_values.get("status", record.get("status"))
                    )
                elif table_name == "it_tickets":
                    update_ticket(
                        conn,
                    ...
                    update_dataset(
                        conn,
                        selected_id,
                        updated_values.get("category", record.get("category"))
                    )
        ...
        with col2:
            if st.form_submit_button("Delete", type="primary"):
                # --- Call delete functions based on table ---
                if table_name == "cyber_incidents":
