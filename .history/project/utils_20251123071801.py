...
def update_delete_record(conn, table_name):
    ...
    # ------- Get records based on selected domain -------
    if table_name == "cyber_incidents":
    ...
    with st.form("update_delete_form"):
        ...
        for field, value in record.items():
            ...
            # Primary key stays locked
            if field == key:
                updated[field] = st.text_input(field, value, disabled=True)

            # Number input if numeric
            elif isinstance(value, (int, float)):
                updated[field] = st.number_input(field, value=float(value))

            # Default text input
            else:
                updated[field] = st.text_input(field, value)
    ...
    if update_button:
        ...
        if table_name == "cyber_incidents":
            update_incident_type(conn, selected, updated["incident_type"])
update_incident_severity(conn, selected, updated["severity"])
update_incident_description(conn, selected, updated["description"])
update_incident_reported_by(conn, selected, updated["reported_by"])
update_incident_status(conn, selected, updated["status"])
