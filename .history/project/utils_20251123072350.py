...
def update_delete_record(conn, table_name):
    ...
    with st.form("update_delete_form"):
        ...
        if table_name == "cyber_incidents":
            # All editable fields for cyber incidents
            st.subheader("Edit Cyber Incident")
            for field in record:
                if field == key:
                    updated[field] = st.text_input(field, record[field], disabled=True)
                elif field == "status":
                    updated[field] = st.selectbox(
                        "Status", ["Open", "In Progress", "Closed", "Resolved"],
                        index=["Open","In Progress","Closed", "Resolved"].index(record[field])
                    )
                elif field == "severity":
                    updated[field] = st.selectbox(
                        "Severity", ["Low", "Medium", "High"],
                        index=["Low","Medium","High"].index(record[field])
                    )
                elif field == "incident_type":
                    updated[field] = st.text_input("Incident Type", record[field])
                elif field == "description":
                    updated[field] = st.text_area("Description", record[field])
                elif field == "reported_by":
                    updated[field] = st.text_input("Reported By", record[field])
                else:
                    updated[field] = st.text_input(field, record[field], disabled=True)

        else:
            # Editable form for tickets and datasets
            for field, value in record.items():
                if field == key:
                    updated[field] = st.text_input(field, value, disabled=True)
                elif isinstance(value, int):
                    updated[field] = int(st.number_input(field, value=int(value)))
                elif isinstance(value, float):
                    updated[field] = float(st.number_input(field, value=float(value)))
                else:
                    updated[field] = st.text_input(field, "" if value is None else value)
    ...
    if update_btn:
        try:
            if table_name == "cyber_incidents":
                # Only updating existing SQL function available: status
                update_incident_status(conn, selected, updated.get("status"))
                st.success("Incident updated successfully!")
            elif table_name == "it_tickets":
                update_ticket(conn, selected, updated)
                st.success("Ticket updated successfully!")
            elif table_name == "datasets_metadata":
                update_dataset(conn, selected, updated)
