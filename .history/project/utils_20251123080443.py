        ...
        if table_name == "cyber_incidents":
            ...
            for field, value in record.items():
                if field == key:
                    ...
                    current_status = value if value in status_options else "Open"
            ...
            for field, value in record.items():
                if field == key:
                    updated[field] = st.text_input(field, value, disabled=True)
                elif field == "priority":  # Only editable field
                    status_options = ["Low", "Medium", "High", "Critical"]
                    current_status = value if value in status_options else "Medium"
                    updated[field] = st.selectbox("Priority", status_options, index=status_options.index(current_status))
                else:  # Display-only fields
                    st.text_input(field, value, disabled=True)
        ...
        try:
            if action == "Update":
                if table_name == "cyber_incidents":
                    ...
                    st.success("Incident '{selected}' updated successfully!")
                elif table_name == "it_tickets":
                    update_ticket(conn, selected, updated)
                    st.success("Ticket '{selected}' updated successfully!")
                elif table_name == "datasets_metadata":
                    update_dataset(conn, selected, updated.get("record_count"))
                    st.success("Dataset '{selected}' updated successfully!")
