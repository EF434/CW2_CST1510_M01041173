...
def data_visualization(conn, table_name):
    ...
    # 6️⃣ Resolution Time by Incident Type
    st.subheader("Resolution Time by Incident Type")
    df_resolution = resolution_time_by_type(conn)
    if not df_resolution.empty:
        fig6 = px.scatter(df_resolution, x="incident_type",
                          y=df_resolution["resolution_days"].dt.days,
                          size=df_resolution["resolution_days"].dt.days,
                          color="incident_type",
                          title="Resolution Time per Incident (days)")
        st.plotly_chart(fig6, use_container_width=True)
    else:
        st.info("No resolved incidents to calculate resolution time.")
