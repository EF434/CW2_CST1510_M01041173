...
def view_records(conn, table_name):
    if table_name == "cyber_incidents":
        ...
        st.markdown(
            ...
            <div class="cyber-sub">🔒 Cybersecurity Incidents</div>
            """,
    ...
    # 3️⃣ Last Updated Date Filter
    min_date = df["last_updated"].min()
    max_date = df["last_updated"].max()

    last_updated_filter = st.sidebar.date_input(
        "Last Updated Range",
    ...
    filtered_df = df[
        ...
        (df["last_updated"] >= pd.to_datetime(last_updated_filter[0])) &
        (df["last_updated"] <= pd.to_datetime(last_updated_filter[1]))
    ]
    else:
