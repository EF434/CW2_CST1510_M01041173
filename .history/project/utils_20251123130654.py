...
def view_records(conn, table_name):
    st.markdown(
            ...
            <div class="cyber-sub">🔒 {table_name}</div>
            """,
...
def view_records(conn, table_name):
    if table_name == "cyber_incidents":
        ...
        record_count = st.sidebar.slider(
            ...
            value=(int(df["record_count"].min()), int(df["record_count"].max()))
        )
