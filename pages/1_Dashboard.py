...
# Info module path
# ----------------------------
INFO_PATH = Path(__file__).parent.parent / "info"
if str(INFO_PATH) not in sys.path:
    sys.path.append(str(INFO_PATH))
 
from info.incidents import (
    insert_incident,
    get_all_incidents,
...
if selected_domain == "Cybersecurity":
    ...
    if choice == menu[0]:
        ...
        with st.form("incident_form"):
            ...
            if submitted:
                row_id = insert_incident(conn, str(date), incident_type, severity, status, description, reported_by)
                if row_id:
